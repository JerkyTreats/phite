...
find . -type f \
    ...
    if grep -l "github.com/JerkyTreats/PHITE/vital-hearth/backend\|VITAL_HEARTH_BACKEND\|.vital-hearth\|Vital Hearth API\|https://api.vital-hearth.internal.jerkytreats.dev\|jerkytreats\|vital-hearth-api" "$file" > /dev/null 2>&1; then
        echo "  📝 Processing: $file"
        
        # Use sed to replace only our template variables (not GitHub Actions or Docker metadata patterns)
        sed -i.bak \
            -e "s|github.com/JerkyTreats/PHITE/vital-hearth/backend|$MODULE_NAME|g" \
            -e "s|VITAL_HEARTH_BACKEND|$PROJECT_NAME|g" \
            -e "s|.vital-hearth|$CONFIG_DIR|g" \
            -e "s|Vital Hearth API|$API_TITLE|g" \
            -e "s|https://api.vital-hearth.internal.jerkytreats.dev|$API_BASE_URL|g" \
            -e "s|jerkytreats|$DOCKER_USERNAME|g" \
            -e "s|vital-hearth-api|$DOCKER_IMAGE_NAME|g" \
            "$file"
